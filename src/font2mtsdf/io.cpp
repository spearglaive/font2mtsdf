...
namespace f2m {
	...
	acma::result<void> write_to_output_from(std::string output_path, std::vector<std::array<std::byte, decoder::font_texture::size_bytes>> bytes) {
		...
		const std::string writer_header = std::format("{} v{}.{}.{}",
			program_info::name,
			program_info::version[0],
			program_info::version[1],
			program_info::version[2]
		);
		__D2D_KTX_VERIFY(ktxHashList_AddKVPair(&ktx_ptr->kvDataHead, KTX_WRITER_KEY, writer_header.size() + 1, writer_header.c_str()));
